...
class GPIO_Util():
   ...
   def __push_the_button(self, gpio_pin):
      self.msg('Attempting to push the button on GPIO %s' % str(gpio_pin), 'debug')
      GPIO.output(gpio_pin, GPIO.HIGH)
      time.sleep(1)
      GPIO.output(gpio_pin, GPIO.LOW)
      self.msg('Successfully pushed the button on GPIO %s' % str(gpio_pin), 'debug')

   def __get_pin(self, func_name):
      self.msg('Attempting to get the GPIO pin number for %s' % func_name, 'debug')
      result = self.gpio_dict[func_name]['pin']
      self.msg('GPIO pin number for %s is %s' % (func_name, str(result)), 'debug')
      return result

   def __proximity_sensor_read_raw(self, name):
      self.msg('Attempting to read raw %s sensor data' % name, 'debug')
      if name == 'CUP':
         self.msg('Turning on the CUP sensor', 'debug')
         GPIO.output(self.__get_pin('WTR_SNS_CS'), GPIO.LOW)
         GPIO.output(self.__get_pin('CUP_SNS_CS'), GPIO.HIGH)
         self.msg('CUP sensor is now ON', 'debug')
      elif name == 'WATER':
         self.msg('Turning on the WATER sensor', 'debug')
         GPIO.output(self.__get_pin('CUP_SNS_CS'), GPIO.LOW)
         GPIO.output(self.__get_pin('WTR_SNS_CS'), GPIO.HIGH)
         self.msg('WATER sensor is now ON', 'debug')
      self.msg('Reading raw value from %s sensor' % name, 'debug')
      value = 0
      for x in range(3):
         raw = VL6180X(1).read_distance()
         if raw == None:
            raw = 255
         self.msg('Raw reading of pass %s: %s' % (str(x+1), str(raw)), 'debug')
         value += raw
      value /= 3
      self.msg('Average reading of 3 passes: %s' % str(value), 'debug')
      self.msg('Turning off all sensors', 'debug')
      GPIO.output(self.__get_pin('CUP_SNS_CS'), GPIO.LOW)
      GPIO.output(self.__get_pin('WTR_SNS_CS'), GPIO.LOW)
      self.msg('All sensors is now OFF', 'debug')
      return value

   def make_coffee(self, option):
      self.msg('Attempting to start making coffee: %s' % option, 'debug')
      if option in AVAILABLE_COFFEE_TYPE_LIST:
         self.__push_the_button(self.__get_pin(option))
         self.msg('Coffee making process initiated successfully', 'debug')
         return 'SUCCESS'
      else:
         self.msg('Start making coffee failed: unknown coffee option %s' % option, 'debug')
         return 'ERROR: unknown coffee option %s' % option

   def get_cup_presence(self):
      self.msg('Attempting to determine cup presence', 'debug')
      result = (self.__proximity_sensor_read_raw('CUP') < CUP_DISTANCE_THRESHOLD)
      self.msg('Determined CUP presence: %s' % str(result), 'debug')
      return result

   def get_water_level(self):
      self.msg('Attempting to determine water level', 'debug')
      raw_reading = self.__proximity_sensor_read_raw('WATER')
      self.msg('WATER sensor raw value received: %s' % str(raw_reading), 'debug')
      ...
      for convert in WATER_LEVEL_TABLE:
         if raw_reading <= convert['max_distance']:
            ...
            break
      self.msg('Translated water level: %s%%' % str(water_level), 'debug')
      return water_level
