...
class Machine():
   ...
   def get_machine_state(self):
      self.msg('Attempting to update machine state, current machine state: %s' % str(self.machine_state), 'debug')
      if self.machine_state == 'READY_TO_MAKE':
         if not self.__is_cup_present() or not self.__is_water_enough():
            self.msg('Cup not present of water not enough, changing machine state to \'NOT_READY_TO_MAKE\'', 'debug')
         ...
         if self.__is_cup_present() and self.__is_water_enough():
            self.msg('Cup is present and water is enough, changing machine state to \'READY_TO_MAKE\'', 'debug')
         ...
         if self.__is_cup_present() and self.__is_water_enough():
            ...
            self.msg('Cup not present of water not enough, changing machine state to \'NOT_READY_TO_MAKE\'', 'debug')
            self.__set_machine_state('NOT_READY_TO_MAKE')
      self.msg('Updated machine state: %s' % self.machine_state, 'debug')
      return self.machine_state

   def get_sensors_status(self, update = False):
      self.msg('Attempting to %s sensor status' % ('update' if update else 'retrieve'), 'debug')
      if update == True:
      ...
      self.msg('Sensor status retrieved: %s' % str(result), 'debug')
      return result

   def get_not_ready_reason(self):
      self.msg('Attempting to retrieve machine not ready reason', 'debug')
      if self.machine_state == 'NOT_READY_TO_MAKE':
         ...
         result = 'please consume the previously made cofee first'
      elif self.machine_state == 'MAKING_IN_PROGRESS':
      ...
      self.msg('Not ready reason: %s' % result, 'debug')
      return result

   def start_make_coffee(self, coffee_type):
      self.msg('Attempting to initiate making coffee %s' % str(coffee_type), 'debug')
      if coffee_type not in AVAILABLE_COFFEE_TYPE_LIST:
         self.msg('Making coffee failed: unknown coffee type %s' % str(coffee_type), 'debug')
         return 'ERROR: unknown coffee type ' + coffee_type
      if self.get_machine_state() != 'READY_TO_MAKE':
         reason = self.get_not_ready_reason()
         self.msg('Making coffee failed: %s' % reason, 'debug')
         return 'ERROR: ' + reason
      result = self.gpio_util.make_coffee(coffee_type)
      if result != 'SUCCESS':
         self.msg('Making coffee failed: %s' % result, 'debug')
         return result
      self.__set_machine_state('MAKING_IN_PROGRESS')
      self.msg('Starting child thread to monitor coffee progress', 'debug')
      self.child_thread.start()
      self.msg('Child thread started, coffee is on the way', 'debug')
      return 'SUCCESS'

   def __set_machine_state(self, new_state):
      self.msg('Attempting to set new machine state: %s' % str(new_state), 'debug')
      if new_state not in ['READY_TO_MAKE', 'NOT_READY_TO_MAKE', 'COFFEE_IS_READY', 'MAKING_IN_PROGRESS']:
         self.msg('%s is not a valid machine state, raising exception' % str(new_state), 'debug')
         raise ValueError()
      with self.machine_state_lock:
         self.machine_state = new_state
      self.msg('New machine state successfully set', 'debug')
      return 'SUCCESS'

   def __is_cup_present(self, update_machine_state = False):
      self.msg('Attempting to update cup presence %s' % (' and update machine state accordingly' if update_machine_state else ''), 'debug')
      ...
      self.msg('Cup presence updated: %s' % str(self.cup_present), 'debug')
      if update_machine_state and (not self.cup_present) and (self.machine_state == 'COFFEE_IS_READY'):
         self.__set_machine_state('NOT_READY_TO_MAKE')
         self.msg('Machine state updated: %s' % self.machine_state, 'debug')
      return self.cup_present

   def __is_water_enough(self, threshold = WATER_THRESHOLD):
      self.msg('Attempting to update water sufficiency', 'debug')
      new_value = (self.__get_water_level() >= threshold)
      with self.water_enough_lock:
         self.water_enough = new_value
      self.msg('Water sufficiency compared to threshold %s%% updated: %s' % (str(threshold), str(self.water_enough)), 'debug')
      return self.water_enough

   def __get_water_level(self):
      self.msg('Attempting to update water level', 'debug')
      ...
      self.msg('Water level updated: %s%%' % str(self.water_level), 'debug')
   ...
   def __keep_update_coffee_ready_status(self):
      self.msg('Started monitoring coffee making progress continuously', 'debug')
      self.dummy_timer = 0
      while self.machine_state == 'MAKING_IN_PROGRESS':
         if self.thread_exit:
            self.msg('Received terminate signal, child thread exiting', 'debug')
            return
         if self.__is_coffee_ready():
            break
         time.sleep(1)
      self.msg('Detected coffee making progress completion', 'debug')
      self.__set_machine_state('COFFEE_IS_READY')
      self.msg('Sending coffee ready push notification', 'debug')
   ...
   def __keep_update_coffee_pickup_status(self):
      self.msg('Started monitoring coffee pickup continuously', 'debug')
      while self.machine_state == 'COFFEE_IS_READY':
         ...
         if not self.__is_cup_present():
            break
         time.sleep(1)
      self.msg('Detected coffee has been picked up', 'debug')
      self.__set_machine_state('NOT_READY_TO_MAKE')
